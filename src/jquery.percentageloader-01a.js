...
(function($) {    
	...
	var items = [valueText, percentageText]
	for (var i in items) {
	    var item = items[i];
	    var selectors =
	       ['-webkit-user-select',
	...
	/* Add the new dom elements to the containing div */
        ...
        /* Status values for our progress bar. Progress is the actual progress
	 * as shown by the central percent value and the inner ring; value
	 * is the text label shown at the bottom and can be set to anything */
        var progress;
        var value;
	...
	if (params['controllable'] === true) {
	    ...
	    function adjustProgressWithXY(x, y) { 
		...
		var startTouchAngle = startAngle - (Math.PI * 2.0);		    
		var range = endAngle - startAngle;
		var posValue = (angle - startTouchAngle) / range;
